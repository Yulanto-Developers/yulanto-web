        ...
        {gsapReady &&
          images.map((image, index) => (
            <div
              ...
              style={{
                zIndex: inPlace === index ? index : images.length + 1,
              }}
            >
              <GalleryImage
                total={images.length}
...
function Tabs({ images, onSelect }: TabsProps) {
  ...
  return (
    ...
    >
      {images.map((image, index) => (
        <g key={`${image.url}-${index}`} className="gallery-tab">
          <defs>
            <clipPath id={`tab_${index}_clip`}>
              <circle cx={getPosX(index)} cy={getPosY()} r={circleRadius} />
            </clipPath>
          </defs>

          <image
            x={getPosX(index) - circleRadius}
